# 팀 스터디 허브 — 첨부파일(PDF·이미지) 구현 계획 (보류)

> ## 이 계획은 지금 실행하지 않는다
>
> 2단계 범위가 마크다운 파일 불러오기로 바뀌면서(2026-08-05) 이 계획은 뒤로 미뤘다.
> 팀이 실제로 필요로 한 것은 파싱이 필요 없는 `.md` 텍스트를 브라우저가 그대로
> 읽어 본문 칸에 채우는 것이었다. PDF·이미지처럼 **파싱이 필요한** 일반 첨부는
> 나중에 필요해지면 이 계획을 그대로 꺼내 쓴다.
>
> 현재 진행 중인 2단계 계획: `docs/2026-08-05-study-hub-phase2-plan.md`

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** 노트에 파일(PDF·이미지 등)을 첨부하고 팀원끼리 내려받을 수 있게 한다.
...
**Tech Stack:** Next.js 15 (App Router, TypeScript), Supabase Storage, Tailwind, Vitest
...
현재 테스트: 단위 20개, 권한 8개. (이 문서를 다시 꺼낼 때는 그 시점의 실제 개수로 갱신할 것)
...
Run: `npm test`
Expected: PASS — 이 문서를 다시 꺼내 쓰는 시점의 기존 테스트 개수 + 17개
...
(3) 본문 블록과 수정·삭제 블록 **사이에** 첨부 섹션을 넣는다:
...
2. **수동 확인** (아래)
3. **배포** — GitHub에 push하면 Vercel이 자동 배포한다
