...
> ## ⚠️ 이 계획서는 범위 변경으로 폐기 대기 중 (2026-08-05 결정)
>
> **아래 내용을 그대로 실행하지 말 것.** 다시 쓰기로 했다.
>
> **왜 바뀌었나:** 팀의 실제 사용 방식이 "웹에서 글을 쓴다"가 아니라
> **"클로드가 정리해준 `.md` 파일을 올린다"** 로 확인됐다.
>
> `.md` 는 그냥 텍스트다. PDF·손글씨와 달리 파싱이 필요 없다. 그래서 파일을
> 스토리지에 보관하고 나중에 읽는 구조가 아니라, **올리는 순간 브라우저가
> 읽어서 본문 입력칸을 채우는** 방식이 맞다. 서버·스토리지·권한 두 겹이
> 전부 불필요해진다. 본문이 DB에 텍스트로 들어가므로 3단계 AI 정리본도
> 그대로 살아 있다.
>
> **새 2단계 범위 (태스크 3개 예상):**
> 1. 마크다운 파일 해석 유틸 — 첫 줄 `# 제목` 추출, 없으면 파일명 사용. 순수 함수 + 단위 테스트
> 2. 노트 작성·수정 폼에 "마크다운 파일 불러오기" 버튼 — 제목·본문 자동 채움. **직접 타이핑도 그대로 유지**, 불러온 뒤 수정 가능
> 3. 본문 마크다운 렌더링 (아래 Task 5를 그대로 재사용)
>
> **일반 첨부(PDF·이미지)는 이번 범위에서 뺀다.** 필요해지면 그때 별도 계획으로.
> 아래 Task 1~4(스토리지·업로드·다운로드·권한 테스트)는 그때 다시 꺼내 쓴다.
>
> ### 3단계 시작 전 준비물 (아직 안 함)
>
> - **`service_role` 키 재발급** — 채팅에 노출된 적 있음. 1단계 코드는 이 키를
>   쓰지 않지만 **3단계 크론이 실제로 사용한다.** 반드시 교체할 것.
>   Supabase → API Keys에서 revoke 후 재발급 → Vercel 환경변수와 `.env.local` 갱신
> - **Gemini API 키 발급** — aistudio.google.com, 무료, 2분
